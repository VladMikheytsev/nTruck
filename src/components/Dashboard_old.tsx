...
import { TransferRequest, TransferStatus, Vehicle } from '../types';
...
const Dashboard: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'requests' | 'routes' | 'sender-groups' | 'receiver-groups'>('requests');
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-600 mt-1">Обзор системы управления складскими операциями</p>
      </div>
...
const TransferRequestCard: React.FC<TransferRequestCardProps> = ({
  ...
  if (isCompact) {
    return (
      <div className={`card p-4 h-full flex flex-col ${deadlineWarning === 'overdue' ? 'border-red-200 bg-red-50' : 
                                      deadlineWarning === 'approaching' ? 'border-yellow-200 bg-yellow-50' : ''}`}>
        {/* Header */}
        <div className="flex items-center justify-between mb-3">
          ...
          {deadlineWarning && (
            <span className={`inline-flex items-center px-1.5 py-0.5 rounded-full text-xs font-medium ${
              deadlineWarning === 'overdue' ? 'bg-red-100 text-red-800' : 'bg-yellow-100 text-yellow-800'
            }`}>
          ...
          {(request.sourceDepartureTime || request.destinationArrivalTime) && (
            <div className="text-xs space-y-1">
              {request.sourceDepartureTime && (
                <div className="text-blue-600 bg-blue-50 px-1.5 py-0.5 rounded truncate">
                  Выезд: {request.sourceDepartureTime}
                </div>
              )}
              {request.destinationArrivalTime && (
                <div className="text-green-600 bg-green-50 px-1.5 py-0.5 rounded truncate">
        ...
        <div className="text-xs text-gray-600 mb-3">
          <span className="font-medium">{request.items.length} поз.</span>
          <div className="truncate mt-1">
            {request.items.slice(0, 1).map(item => item.name).join(', ')}
  ...
  return (
    <div className={`card p-6 ${deadlineWarning === 'overdue' ? 'border-red-200 bg-red-50' : 
                                deadlineWarning === 'approaching' ? 'border-yellow-200 bg-yellow-50' : ''}`}>
      <div className="flex items-start justify-between">
        <div className="flex-1">
          ...
          <div className="space-y-2 mb-3">
              ...
              <div className="flex items-center space-x-2">
                ...
                {request.sourceDepartureTime && (
                  <span className="text-xs text-blue-600 bg-blue-50 px-2 py-1 rounded">
                    Выезд со склада ({getWarehouseName(request.sourceWarehouse)}): {request.sourceDepartureTime}
                  </span>
                )}
              </div>
            </div>
              ...
              <div className="flex items-center space-x-2">
                ...
                {request.destinationArrivalTime && (
                  <span className="text-xs text-green-600 bg-green-50 px-2 py-1 rounded">
                    Приезд на склад ({getWarehouseName(request.destinationWarehouse)}): {request.destinationArrivalTime}
                  </span>
                )}
              </div>
            </div>
            ...
            {(request.assignedDriverId || request.assignedVehicle) && (
              <div className="flex items-center space-x-4 text-xs text-gray-600 bg-gray-50 p-2 rounded">
                ...
                {request.assignedVehicle && (
                  <div className="flex items-center space-x-1">
                    ...
                    <span>Vehicle: {request.assignedVehicle}</span>
                  </div>
                )}
              </div>
            )}
        ...
        >
          <span className="text-sm">{expanded ? 'Свернуть' : 'Подробнее'}</span>
        </button>
      </div>
      ...
      {expanded && (
        <div className="mt-6 pt-6 border-t border-gray-200">
          <div className="grid md:grid-cols-2 gap-6">
            {/* Items List */}
            <div>
              <h4 className="text-sm font-medium text-gray-900 mb-3">Позиции для перемещения</h4>
            ...
            {request.notes && (
              <div>
                <h4 className="text-sm font-medium text-gray-900 mb-3">Примечания</h4>
          ...
          <div className="mt-6 pt-6 border-t border-gray-200">
            <h4 className="text-sm font-medium text-gray-900 mb-3">Информация о заявке</h4>
            <div className="grid md:grid-cols-2 gap-6">
              {/* Created By */}
              <div className="space-y-2">
                <div className="flex items-center space-x-2">
                  <UserIcon className="h-4 w-4 text-green-600" />
                  <span className="text-sm font-medium text-gray-700">Создана пользователем:</span>
                </div>
                <div className="ml-6 text-sm text-gray-600">
                  {(() => {
                    const creator = state.users.find((u: any) => u.id === request.createdBy);
                    return creator ? `${creator.firstName} ${creator.lastName} (@${creator.login})` : 'Unknown User';
                  })()}
                </div>
              ...
              {/* Updated By */}
              {request.updatedBy && request.updatedAt && (
                <div className="space-y-2">
                  <div className="flex items-center space-x-2">
                    <Edit className="h-4 w-4 text-blue-600" />
                    <span className="text-sm font-medium text-gray-700">Изменена пользователем:</span>
                  </div>
                  <div className="ml-6 text-sm text-gray-600">
                    {(() => {
                      const updater = state.users.find((u: any) => u.id === request.updatedBy);
                      return updater ? `${updater.firstName} ${updater.lastName} (@${updater.login})` : 'Unknown User';
                    })()}
                  </div>
                  <div className="ml-6 text-xs text-gray-500">
          ...
          {currentUser?.role === 'admin' && (
            <div className="mt-6 pt-6 border-t border-gray-200">
              <h4 className="text-sm font-medium text-gray-900 mb-3">Действия администратора</h4>
              <div className="flex flex-wrap gap-3">
                <button
                  onClick={handleEditRequestLocal}
                  className="inline-flex items-center px-3 py-2 text-sm font-medium text-primary-600 bg-primary-50 border border-primary-200 rounded-md hover:bg-primary-100 transition-colors"
                >
